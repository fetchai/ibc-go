...
by the port
[`Router`](https://github.com/cosmos/ibc-go/blob/v3.1.0/modules/core/05-port/types/router.go) on the
IBC module.
...
In order to use IBC, we need to add the new modules to the module `Manager` and to the `SimulationManager` in case your application supports [simulations](https://github.com/cosmos/cosmos-sdk/blob/v0.45.4/docs/building-modules/simulator.md).
...
[`BeginBlock`](https://github.com/cosmos/ibc-go/blob/v3.1.0/modules/core/02-client/abci.go) logic as
well. This is optional as it is only required if your application uses the [localhost
client](https://github.com/cosmos/ibc/blob/82c74c35f237767b533683206c21cb3e59e612ce/spec/client/ics-009-loopback-client) to connect two
...
[`SimApp`](https://github.com/cosmos/ibc-go/blob/v3.1.0/testing/simapp/app.go).
